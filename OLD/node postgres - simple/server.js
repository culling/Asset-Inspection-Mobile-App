...
app.get('/dbSetup', (req, response) => {
  ...
  client.connect();
  let text = "";
  let queryText = `INSERT INTO test_table VALUES (${
    'use strict';
    ...
    app.get('/dbSetup', (req, response) => {
      ...
      client.connect();
      let text = "";
      let queryText = `INSERT INTO test_table VALUES (${Math.floor(Math.random() * 10000) }, '${Date().toString()}') RETURNING *;`;
      client.query(queryText , (err, res) => {
        if (err) throw err;
        for (let row of res.rows) {
          response.write(JSON.stringify(row) +  "\n");
